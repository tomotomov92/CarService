...
namespace BusinessLogic.BLs
{
    public class ClientCarBL : BaseBL<ClientCarDTO>
    {
        ...
        public override async Task<ClientCarDTO> UpdateAsync(ClientCarDTO dto)
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"UPDATE ClientCars SET ClientId = @clientId, CarBrandId = @carBrandId, LicensePlate = @licensePlate, Mileage = @mileage, Archived = @archived WHERE Id = @id;";
            BindId(cmd, dto);
            BindParams(cmd, dto);
            await cmd.ExecuteNonQueryAsync();
            return dto;
        ...
        public override ClientCarDTO Get(int id)
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"
...
WHERE ClientCars.Id = @id;";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"DELETE FROM ClientCars WHERE Id = @id;";
            BindId(cmd, id);
            await cmd.ExecuteNonQueryAsync();
        }
...
ORDER BY ClientCars.Id;";
        ...
        public override async Task<IEnumerable<ClientCarDTO>> GetAllActiveAsync()
        {
